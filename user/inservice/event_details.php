...
$user_checklist_items_is = ['before' => [], 'after' => []];
$shared_content_is = [];
$errors_is_detail = [];
...
// CSRF for checklist item status update
...
if (!$inservice_department_id_detail) $inservice_department_id_detail = 2; // Fallback, should be configured
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_checklist_item_status'])) {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '', 'inservice_event_detail_action_' . $event_id_is_detail)) {
        ...
        // The value of checkbox 'is_completed' will be its 'value' attribute (1) if checked, or not present in POST if unchecked.
        $new_is_completed_status = isset($_POST['is_completed_chk_' . $checklist_id_to_update]) ? 1 : 0;

        if ($checklist_id_to_update) {
            // Ensure this user is responsible for this checklist item
            ...
            $csrf_token_event_detail_actions = regenerate_csrf_token('inservice_event_detail_action_' . $event_id_is_detail);
            if(empty($errors_is_detail)) { header("Location: event_details.php?event_id=" . $event_id_is_detail); exit; }
...
if($stmt_user_chk){
    ...
    while($uchk_row = $res_uchk->fetch_assoc()) {
        if(isset($user_checklist_items_is[$uchk_row['ItemType']])) {
...
<?php if (isset($_SESSION['flash_message'])) { $flash_is_det_user = $_SESSION['flash_message']; echo "<div class='alert alert-{$flash_is_det_user['type']} alert-dismissible fade show'>{$flash_is_det_user['text']}<button type='button' class='close' data-dismiss='alert'>&times;</button></div>"; unset($_SESSION['flash_message']); echo "<script> /* Dismiss JS */ </script>";} ?>
<?php if (!empty($errors_is_detail)): ?> <div class="alert alert-danger"><ul><?php foreach ($errors_is_detail as $err_isd_item_user): ?><li><?php echo htmlspecialchars($err_isd_item_user); ?></li><?php endforeach; ?></ul></div> <?php endif; ?>
...
<div class="card-body">
    <?php foreach(['before' => 'قبل از برگزاری', 'after' => 'بعد از برگزاری'] as $type_key_is_user => $type_label_is_user): ?>
        <?php if(!empty($user_checklist_items_is[$type_key_is_user])): ?>
        <h6 class="mt-3 text-muted font-weight-bold"><?php echo $type_label_is_user; ?>:</h6>
        <ul class="list-group">
        <?php foreach($user_checklist_items_is[$type_key_is_user] as $item_chk_u_item_disp): ?>
            <li class="list-group-item d-flex justify-content-between align-items-center <?php if($item_chk_u_item_disp['IsCompleted']) echo 'list-group-item-light';?>">
                <form action="event_details.php?event_id=<?php echo $event_id_is_detail; ?>" method="POST" class="w-100 d-flex justify-content-between align-items-center m-0 p-0">
                    <input type="hidden" name="csrf_token" value="<?php echo $csrf_token_event_detail_actions; ?>">
                    <input type="hidden" name="checklist_id" value="<?php echo $item_chk_u_item_disp['ChecklistID']; ?>">
                    <input type="hidden" name="update_checklist_item_status" value="1">
                    <div>
                        <input type="checkbox" class="form-check-input-inline" name="is_completed_chk_<?php echo $item_chk_u_item_disp['ChecklistID'];?>" value="1" <?php if($item_chk_u_item_disp['IsCompleted']) echo 'checked'; ?> onchange="this.form.submit();" id="chk_user_<?php echo $item_chk_u_item_disp['ChecklistID'];?>">
                        <label for="chk_user_<?php echo $item_chk_u_item_disp['ChecklistID'];?>" class="<?php if($item_chk_u_item_disp['IsCompleted']) echo 'text-decoration-line-through text-muted';?>"><?php echo htmlspecialchars($item_chk_u_item_disp['ItemName']); ?></label>
                        <?php if($item_chk_u_item_disp['DueDate']): ?><small class="d-block text-info">سررسید: <?php echo to_jalali($item_chk_u_item_disp['DueDate'], 'yyyy/MM/dd');?></small><?php endif; ?>
                    </div>
                </form>
                <span class="badge badge-<?php echo $item_chk_u_item_disp['IsCompleted'] ? 'success':'warning';?> badge-pill"><?php echo $item_chk_u_item_disp['IsCompleted'] ? 'انجام شده':'در انتظار';?></span>
            </li>
        <?php endforeach; ?>
        </ul> <?php endif; ?>
    <?php endforeach; ?></div></div>
<?php elseif ($user_id_is_detail && get_current_user_type() === 'teacher'): ?>
...
<div class="card-body">
    <?php if(!empty($shared_content_is)): ?><ul class="list-unstyled">
        <?php foreach($shared_content_is as $file_sh_item_disp): ?>
        <li class="mb-2 pb-2 border-bottom">
            <div class="d-flex align-items-center">
                ...
                <a href="/my_site/<?php echo htmlspecialchars(ltrim($file_sh_item_disp['FilePath'],'/')); ?>" target="_blank" download="<?php echo htmlspecialchars($file_sh_item_disp['FileName']); ?>">
                    <strong><?php echo htmlspecialchars($file_sh_item_disp['FileName']); ?></strong>
                </a>
            </div>
            <?php if($file_sh_item_disp['Description']):?><small class="d-block text-muted ml-4 pl-2"><?php echo htmlspecialchars($file_sh_item_disp['Description']);?></small><?php endif;?>
            <small class="d-block text-muted ml-4 pl-2">تاریخ آپلود: <?php echo to_jalali($file_sh_item_disp['UploadDate'], 'yyyy/MM/dd');?></small>
        </li>
        <?php endforeach; ?></ul>
