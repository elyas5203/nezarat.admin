...
// TODO: Fetch categories if you implement item categories for warehouse:
// $wh_categories_q = $conn->query("SELECT CategoryID, CategoryName FROM WarehouseItemCategories ORDER BY CategoryName");
// $available_wh_categories = [];
// if($wh_categories_q) { while($cat_wh = $wh_categories_q->fetch_assoc()) $available_wh_categories[$cat_wh['CategoryID']] = $cat_wh['CategoryName']; $wh_categories_q->close(); }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_warehouse_item'])) {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '', 'finance_warehouse_action')) {
        ...
        $item_wh_to_edit_values['UnitPrice'] = isset($_POST['unit_price_wh']) && trim($_POST['unit_price_wh']) !== '' ? floatval(preg_replace('/[^0-9.]/', '', $_POST['unit_price_wh'])) : null;
        // $item_wh_to_edit_values['CategoryID'] = ...
        ...
        } else { $errors_wh[] = "خطا در بررسی نام قلم انبار."; }
...
if (isset($_GET['edit_item_id']) && is_numeric($_GET['edit_item_id']) && $_SERVER["REQUEST_METHOD"] != "POST") {
    $edit_id_wh_get = (int)$_GET['edit_item_id'];
    $stmt_edit_wh_get = $conn->prepare("SELECT WarehouseItemID, ItemName, Description, Quantity, UnitPrice, CategoryID FROM WarehouseItems WHERE WarehouseItemID = ?");
    if ($stmt_edit_wh_get) { $stmt_edit_wh_get->bind_param("i", $edit_id_wh_get); $stmt_edit_wh_get->execute(); $result_edit_wh_get = $stmt_edit_wh_get->get_result();
        if ($data_wh_get = $result_edit_wh_get->fetch_assoc()) { $item_wh_to_edit_values = $data_wh_get; $edit_mode_wh = true; }
        else $_SESSION['flash_message'] = ['type' => 'danger', 'text' => "قلم انبار یافت نشد."]; $stmt_edit_wh_get->close();
...
if (isset($_GET['delete_item_id']) && is_numeric($_GET['delete_item_id'])) {
    if (!isset($_GET['csrf_token']) || !verify_csrf_token($_GET['csrf_token'], 'finance_warehouse_action')) {
        ...
        $delete_id_wh_get = (int)$_GET['delete_item_id'];
        // TODO: Add dependency check if warehouse items are logged for usage (e.g., in a WarehouseLogs table)
        $stmt_delete_wh_get = $conn->prepare("DELETE FROM WarehouseItems WHERE WarehouseItemID = ?");
        if ($stmt_delete_wh_get) { $stmt_delete_wh_get->bind_param("i", $delete_id_wh_get);
            if ($stmt_delete_wh_get->execute() && $stmt_delete_wh_get->affected_rows > 0) $_SESSION['flash_message'] = ['type' => 'success', 'text' => "قلم از انبار حذف شد."];
            else $_SESSION['flash_message'] = ['type' => 'danger', 'text' => "خطا حذف قلم: " . $stmt_delete_wh_get->error]; $stmt_delete_wh_get->close();
...
$wh_items_list_q_main = $conn->query("SELECT wi.*, (wi.Quantity * wi.UnitPrice) as TotalValue FROM WarehouseItems wi ORDER BY wi.ItemName LIMIT 100"); // Add pagination later
?>
...
<?php if (isset($_SESSION['flash_message'])) { $flash_wh = $_SESSION['flash_message']; echo "<div class='alert alert-{$flash_wh['type']} alert-dismissible fade show'>{$flash_wh['text']}<button type='button' class='close' data-dismiss='alert'>&times;</button></div>"; unset($_SESSION['flash_message']); echo "<script> /* Dismiss JS */</script>";} ?>
<?php if (!empty($errors_wh)): ?> <div class="alert alert-danger"><ul><?php foreach ($errors_wh as $err_wh_item_msg): ?><li><?php echo htmlspecialchars($err_wh_item_msg); ?></li><?php endforeach; ?></ul></div> <?php endif; ?>
...
<div class="row"><div class="col-lg-5 mb-4"><div class="card shadow-sm">
    ...
    <form action="warehouse.php<?php if($edit_mode_wh && $item_wh_to_edit_values['WarehouseItemID']) echo '?edit_item_id='.$item_wh_to_edit_values['WarehouseItemID']; ?>" method="POST">
        ...
        <div class="form-group"><label for="item_name_wh_form">نام قلم <span class="text-danger">*</span></label><input type="text" class="form-control <?php echo (!empty($errors_wh) && isset($_POST['item_name_wh'])) ? 'is-invalid' : ''; ?>" id="item_name_wh_form" name="item_name_wh" value="<?php echo htmlspecialchars($item_wh_to_edit_values['ItemName']); ?>" required></div>
        <div class="form-row">
            <div class="form-group col-md-6"><label for="quantity_wh_form">تعداد موجود <span class="text-danger">*</span></label><input type="number" class="form-control <?php echo (!empty($errors_wh) && isset($_POST['quantity_wh'])) ? 'is-invalid' : ''; ?>" id="quantity_wh_form" name="quantity_wh" value="<?php echo htmlspecialchars($item_wh_to_edit_values['Quantity']); ?>" min="0" required></div>
            <div class="form-group col-md-6"><label for="unit_price_wh_form">قیمت واحد (تومان)</label><input type="text" class="form-control <?php echo (!empty($errors_wh) && isset($_POST['unit_price_wh'])) ? 'is-invalid' : ''; ?>" id="unit_price_wh_form" name="unit_price_wh" value="<?php echo htmlspecialchars($item_wh_to_edit_values['UnitPrice'] ?? ''); ?>" placeholder="اختیاری، برای ارزش‌گذاری"></div>
        </div>
        <div class="form-group"><label for="description_wh_form">توضیحات</label><textarea class="form-control" id="description_wh_form" name="description_wh" rows="2"><?php echo htmlspecialchars($item_wh_to_edit_values['Description']); ?></textarea></div>
        <!-- TODO: Category select dropdown -->
    ...
    <?php if($wh_items_list_q_main && $wh_items_list_q_main->num_rows > 0): ?><div class="table-responsive"><table class="table table-sm table-striped table-hover">
        ...
        <?php $wh_row_idx = 1; while($wh_item_idx = $wh_items_list_q_main->fetch_assoc()): ?><tr>
            <td><?php echo $wh_row_idx++; ?></td><td><strong><?php echo htmlspecialchars($wh_item_idx['ItemName']);?></strong><small class="d-block text-muted"><?php echo htmlspecialchars(mb_substr($wh_item_idx['Description'] ?? '',0,50)).(mb_strlen($wh_item_idx['Description'] ?? '') > 50 ? '...' : '');?></small></td>
            <td class="text-center font-weight-bold"><?php echo $wh_item_idx['Quantity'];?></td>
            <td class="text-center"><?php echo $wh_item_idx['UnitPrice'] ? number_format($wh_item_idx['UnitPrice'],0) . ' ت' : '-';?></td>
            <td class="text-center"><?php echo $wh_item_idx['TotalValue'] ? number_format($wh_item_idx['TotalValue'],0) . ' ت' : '-';?></td>
            <td class="actions-cell">
                <a href="warehouse.php?edit_item_id=<?php echo $wh_item_idx['WarehouseItemID'];?>" class="btn btn-xs btn-warning" title="ویرایش"><svg class="icon" width="12" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></a>
                <a href="warehouse.php?delete_item_id=<?php echo $wh_item_idx['WarehouseItemID'];?>&csrf_token=<?php echo $csrf_token_warehouse; ?>" class="btn btn-xs btn-danger" title="حذف" onclick="return confirm('آیا از حذف این قلم از انبار مطمئن هستید؟');"><svg class="icon" width="12" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></a>
